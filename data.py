...
# 创建训练数据
	def create_train_data(self):
		...
		# fine tune
		np.save(self.npy_path + '/imgs_train_finetune.npy', imgdatas)
		np.save(self.npy_path + '/imgs_mask_train_finetune.npy', imglabels)
...
# 创建测试数据
	def create_test_data(self):
		...
		print(len(imgs))
		imgdatas = np.ndarray((len(imgs),self.out_rows,self.out_cols,1), dtype=np.uint8)
		for midname in range(len(imgs)):
			# midname = imgname[imgname.rindex("/")+1:]
			d = str(midname)
			img = load_img(self.test_path + "/" + d + '.' +self.img_type,color_mode = 'grayscale')		
			img = img_to_array(img)
...
# 加载训练图片与mask
	def load_train_data(self):
		...
		print('-'*30)
		imgs_train = np.load(self.npy_path+"/imgs_train_finetune.npy")
		imgs_mask_train = np.load(self.npy_path+"/imgs_mask_train_finetune.npy")
		...
		# first 5 are drawed membrane
		# imgs_draw = imgs_mask_train[0:5]
		# imgs_draw[imgs_draw > 0.5] = 1
		# imgs_draw[imgs_draw <= 0.5] = 0
		
        # 做一个阈值处理，输出的概率值大于0.5的就认为是annotation，否则认为是don't use
		imgs_mask_train[imgs_mask_train > 0.5] = 1
		imgs_mask_train[imgs_mask_train <= 0.5] = 0
		# combine labels together
		# imgs_mask_train[0:5] = imgs_draw
